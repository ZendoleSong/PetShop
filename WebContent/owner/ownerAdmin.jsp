...
PetOwner owner=(PetOwner)(session.getAttribute("owner"));
session.setAttribute("owner",petOwnerService.find(owner.getOwId()));
request.setAttribute("store",petStoreService.getAllPetStore());
request.setAttribute("stockpet",petService.getAllPetsOnSell());
request.setAttribute("newpet",petService.getAllPetsNew());
request.setAttribute("ownpet",petService.getAllOwnerPets(owner.getOwId()));
%>
...
<ul id="cardArea" class="card-area clearfix">
	<li class="card-item">
		<div class="card">
			<div class="card-title title-even">
				<div class="content"><h1>库存宠物</h1></div>
			</div>
			<div class="card-content content-first">
				<div style="padding:50px;">宠物主人可以在这里购买库存宠物</div>
			</div>
				...
				<div class="main-info">
                	<ul>
                    	...
                    	<li>
                        	<div class="left">
                            	<p class="tit">宠物购买</p>
                                <p class="hys">在这里购买库存宠物</p>
                            </div>
                            <div class="right">
                            	<button class="zoomIn dialog1">立即购买</button>
                            </div>
                        </li>
                    </ul>
				</div>
	...
	<li class="card-item active">
		<div class="card">
			<div class="card-title title-even">
				<div class="content"><h1>新培育宠物</h1></div>
			</div>
			<div class="card-content content-first">
				<div style="padding:50px;">宠物主人可以在这里购买新培育宠物</div>
			</div>
				...
				<div class="main-info">
                	<ul>
                    	...
                    	<li>
                        	<div class="left">
                            	<p class="tit">宠物购买</p>
                                <p class="hys">在这里可以购买新培育宠物</p>
                            </div>
                            <div class="right">
                            	<button class="zoomIn dialog2">立即购买</button>
                            </div>
                        </li>
                    </ul>
				</div>
	...
	<li class="card-item">
		<div class="card">
			<div class="card-title title-even">
				<div class="content"><h1>出售宠物</h1></div>
			</div>
			<div class="card-content content-first">
				<div style="padding:50px;">宠物主人可以在这里出售自己拥有的宠物给宠物商店</div>
			</div>
...
<c:choose>
	<c:when test="${status.count==1}">
	    <c:set  var="showpet"  value="${requestScope.stockpet}"  scope="request"/>
		<c:set  var="div_class"  value="pop1"  scope="page"/>
	    <c:set  var="span_class"  value="pop1-close"  scope="page"/>
	    <c:set  var="title"  value="库存宠物"  scope="page"/>
	</c:when>
	<c:when test="${status.count==2}">
	    <c:set  var="showpet"  value="${requestScope.newpet}"  scope="request"/>
		<c:set  var="div_class"  value="pop2"  scope="page"/>
	    <c:set  var="span_class"  value="pop2-close"  scope="page"/>
	    <c:set  var="title"  value="新培育宠物"  scope="page"/>
	</c:when>
	<c:otherwise>
	    <c:set  var="showpet"  value="${requestScope.ownpet}"  scope="request"/>
	    <c:set  var="div_class"  value="pop3"  scope="page"/>
	    <c:set  var="span_class"  value="pop3-close"  scope="page"/>
	    <c:set  var="title"  value="拥有宠物"  scope="page"/>
	</c:otherwise>
</c:choose >
...
<c:choose>
	<c:when test="${status.count==1}">
	  ...
	  <c:set  var="title"  value="购买库存宠物"  scope="page"/>
	...
	<c:when test="${status.count==2}">
	  ...
	  <c:set  var="title"  value="购买新培育宠物"  scope="page"/>
	...
	<c:otherwise>	  
	  ...
	  <c:set  var="title"  value="卖出已有宠物"  scope="page"/>
		...
		<ul class="list">
			...
			<li>
				...
				<c:forEach items="${requestScope.store}" var="store">
					<option value="${store.stId}">${store.stName }</option>
				</c:forEach>
			    </select>
				</div>
			</li>
			...
			<li>
			    <input onclick="ajax_to('${pageScope.form_id}','dealpetservlet',${sessionScope.owner.money})" type="button" value="${pageScope.button_name}" class="submitBtn" />
			</li>
		</ul>
	</form>
</div><!-- div_id end -->	
...
<div class='box box-3' style="z-index:3;">
  <dl>
    <dd style="">用户登录信息<b id="small_button" class="up"></b></dd>
  	<ul>
   	<li><span>角&nbsp;&nbsp;&nbsp;色：</span><i>宠物主人</i></li>
       <li><span>用户名：</span><i>${sessionScope.owner.owName }</i></li>
       <li><span>余&nbsp;&nbsp;&nbsp;额：</span><i>${sessionScope.owner.money}</i></li>
